...
export default function ProfileView({ user, profile }: { user: any, profile: any }) {
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                        <div className="relative -mt-16 flex items-end">
                            <div className="relative h-32 w-32 overflow-hidden rounded-2xl border-4 border-white bg-slate-100 shadow-md">
                                ...
                                />
                                <button className="absolute bottom-2 right-2 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900/60 text-white backdrop-blur-sm transition-colors hover:bg-slate-900/80" title="Cambiar foto (Próximamente)">
                                    <Camera className="h-4 w-4" />
                                </button>
                            </div>
                            <div className="ml-6 pb-2">
                                <h1 className="text-2xl font-bold text-slate-900">{profile?.full_name || "Usuario Administrador"}</h1>
                                <p className="text-sm font-medium text-slate-500 flex items-center gap-1.5 mt-1">
                                    <Shield className="h-4 w-4 text-green-600" />
                                    {profile?.role === 'ADMIN' ? 'Administrador General' : profile?.role}
                                </p>
                            </div>
                        </div>
                ...
                <div className="col-span-1 lg:col-span-2 space-y-6">
                    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <div className="mt-1 font-medium text-slate-900">En desarrollo</div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-500">Cargo</label>
                                <div className="mt-1 font-medium text-slate-900">{profile?.role}</div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-500">Departamento</label>
                                <div className="mt-1 font-medium text-slate-900">SANA Plataforma</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3 rounded-lg border border-slate-100 p-3 bg-slate-50 opacity-60">
                                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm text-slate-500">
                                    <Phone className="h-5 w-5" />
                                </div>
                                <div>
                                    <p className="text-xs font-medium text-slate-500">Teléfono móvil (Próximamente)</p>
                                    <p className="text-sm font-medium text-slate-900">No vinculado</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="col-span-1 space-y-6">
                    <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div className="space-y-5">
                            {[
                                { title: "Perfil cargado", desc: "El Sistema inicializó tu vista", time: "Recién" },
                                { title: "Sesión iniciada", desc: "Accediste al sistema con credenciales", time: "Hoy" }
                            ].map((activity, i) => (
                                <div key={i} className="flex gap-4">
                                    <div className="flex flex-col items-center">
                                        ...
                                        {i !== 1 && <div className="h-full w-px bg-slate-200 mt-2"></div>}
                                    </div>
                                    <div className="pb-4">
